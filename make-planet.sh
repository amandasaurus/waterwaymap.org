...
make planet-waterway-boatable.pmtiles planet-waterway-all.pmtiles planet-waterway-river-stream.pmtiles planet-waterway-name-group-name.pmtiles
...
jq <./docs/data/tilesets.json '.tilesets[3].key = "planet-waterway-name-group-name"|.tilesets[3].text = "Named Waterways (<code>waterway</code> &amp; <code>name*</code> tags, grouped by <code>wikidata</code> then <code>name</code>)"' | sponge ./docs/data/tilesets.json
